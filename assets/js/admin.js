...
const renderInventoryRow = (product) => {
    return `
        <tr class="border-b border-gray-700/50 hover:bg-white/5 transition-colors">
            ...
            <td class="p-4">
                <button class="edit-product-btn text-gray-400 hover:text-white transition-colors" data-id="${product.id}">
    ...
    if (ordersBody) {
        ...
        document.querySelectorAll('.status-select').forEach(select => {
            select.addEventListener('change', (e) => {
                ...
                UI.showToast(`Order ${orderId} updated to ${newStatus}`, 'info');
            });
        });
    }
};
    ...
    UI.showModal({
        ...
        content: `
            <div class="space-y-4">
                <div>
                    ...
                    <input type="number" id="edit-price" value="${product.price}" class="w-full bg-dark-gray border border-gray-600 rounded p-2 text-white outline-none focus:border-electric-cyan">
                </div>
                <div>
                    <label class="block text-sm text-gray-400 mb-1">Stock Level</label>
                    <input type="number" id="edit-stock" value="${product.stock}" class="w-full bg-dark-gray border border-gray-600 rounded p-2 text-white outline-none focus:border-electric-cyan">
                </div>
            </div>
        `,
        ...
        onConfirm: () => {
            ...
            if (isNaN(price) || isNaN(stock)) {
                UI.showToast('Invalid input values', 'error');
                return false;
            }

            store.updateProduct(productId, { price, stock });
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', (e) => {
            ...
            if (store.login(username, password)) {
                ...
                UI.showToast('Access Denied. Invalid credentials.', 'error');
            }
        });
    }
