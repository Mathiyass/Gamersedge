    ...
    return `
        <div class="holo-card glass-effect p-4 rounded-xl flex flex-col group" data-id="${product.id}">
            <div class="relative h-48 w-full overflow-hidden rounded-lg mb-4 bg-white/5 flex items-center justify-center">
                <img src="${product.image}" alt="${product.name}" class="object-contain h-full w-full transition-transform duration-500 group-hover:scale-110" loading="lazy">
...
const initUIControls = () => {
    ...
    if (checkoutBtn) {
        checkoutBtn.addEventListener('click', () => {
            ...
            // Industrial Grade Checkout Form
            UI.showModal({
                ...
                content: `
                    <form id="checkout-form" class="space-y-4">
                        <div>
                            ...
                            <input type="text" id="cust-name" class="w-full bg-dark-gray border border-gray-600 rounded p-2 text-white focus:border-electric-cyan outline-none" placeholder="John Doe">
                        </div>
                        <div>
                            <label class="block text-sm text-gray-400 mb-1">Email</label>
                            <input type="email" id="cust-email" class="w-full bg-dark-gray border border-gray-600 rounded p-2 text-white focus:border-electric-cyan outline-none" placeholder="john@example.com">
                        </div>
                        <div>
                            <label class="block text-sm text-gray-400 mb-1">Shipping Address</label>
                            <textarea id="cust-address" class="w-full bg-dark-gray border border-gray-600 rounded p-2 text-white focus:border-electric-cyan outline-none" placeholder="Colombo, Sri Lanka"></textarea>
                        </div>
                        <div class="text-right text-sm text-gray-500 mt-2">Total: <span class="text-white font-bold">${formatCurrency(store.getCartTotal())}</span></div>
                    </form>
                `,
                ...
                onConfirm: () => {
                    ...
                    const order = store.createOrder({ name, email, address });
                    UI.showToast(`Order Placed! ID: ${order.id}`, 'success');
                    toggleCart();
